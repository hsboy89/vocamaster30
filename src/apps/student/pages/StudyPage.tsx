...
export function StudyPage({ level, day, category, onBack, onQuizStart }: StudyPageProps) {
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            await generateDayVocaPDF(level, day, words, category);
        } catch (error) {
            ...
            <header className="sticky top-0 z-40 bg-[#020617]/80 backdrop-blur-xl border-b border-white/5">
                <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <button
                        ...
                        className="group flex items-center gap-2 text-slate-400 hover:text-white transition-all disabled:opacity-50"
                        title="PDF 다운로드"
                    >
