import { Level, StudyStatus, LEVEL_INFO } from '../../types';
...
export function DayGrid({ level, onDaySelect, onOpenWrongNote }: DayGridProps) {
        ...
        <div className="max-w-6xl mx-auto px-4 py-6">
            ...
            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
                {days.map((day) => {
                    ...
                    const hasData = true; // 모든 30일 데이터 생성됨

                    return (
                        <button
                            key={day}
                            onClick={() => hasData && onDaySelect(day)}
                            disabled={!hasData}
                            className={`day-card group ${getStatusClass(status)} ${!hasData ? 'opacity-40 grayscale cursor-not-allowed' : ''
                                }`}
                        >
